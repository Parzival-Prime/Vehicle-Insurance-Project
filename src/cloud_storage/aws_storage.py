...
class SimpleStorageService:
    """
    Aclass to interact wtith AWS s3 storage, providing methods for file management, data uploads, data retrieval in S3 Bucket 
    """
    
    def __init__(self):
        """Initializes the SimpleStorageService instance with s3 resource and client from the S3Client."""
    ...
    def read_object(object_name: str, decode: bool=True, make_readable: bool=False) -> Union[StringIO, str]:
        ...
        try:
            # Read and decode content if decode=True
            func = (
                lambda: object_name.get()['Body'].read().decode() if decode else object_name.get()['Body'].read()
            )
            
            # Convert content into readable form if makek_readable=True
